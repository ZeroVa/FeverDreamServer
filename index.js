...
app.get("/generateBackground", async (c) => {
  ...
  let writtenImage = fs.promises.appendFile(
    `./tmp/bg-${new Date().getTime()}.png`,
    image
  );
...
app.get("/image/:id", async (c) => {
  ...
  let image = fs.readFileSync(`./assets/objects/${imageId}.png`);
  c.status(200);
  c.header("Content-Type", "image/png");
  return c.body(Buffer.from(image));
});
...
app.get("/game", async (c) => {
  let gameJsonImport = await import("./gameStuff.json", {
    assert: { type: "json" },
  });
  const gameJson = gameJsonImport.default;
  c.status(200);
  return c.json(gameJson);
});
